...
- Overview of the standard workflow
...
.pull-left[
![Screenshot](Figures/Lopez-Anido.png)

*Lopez-Anido CB, Vatén A, Smoot NK, Sharma N et al. Dev Cell 2021 Apr 5;56(7):1043-1055.e4.*


]
.pull-right[
```{r Ara leaf, echo=FALSE, out.width = '80%'}
...
You will test the standard pipeline workflow in the computer practice: 
...
knitr::include_graphics("Figures/DevelopmentalTrajectory.png")
```
Pseudotime should reflect to developmental gradients in plant tissue
]
.pull-right[
##Challenges
...
✅   Different differentiation pace  in different cell types
...
---
#Slingshot: authors use case

.pull-left[
##Model of stomatal lineage
```{r slingshot authors, out.width= "80%", echo = FALSE}
knitr::include_graphics("Figures/slingshot_notation.png")
```
]
.pull-right[
##Single-cell reconstruction of stomatal lineage
```{r slingshot authors 2, out.width= "80%", echo = FALSE}
knitr::include_graphics("Figures/slingshot_authors.png")
```
]

---
#scVelo - RNA Velocity-Based Pseudotime

.pull-left[
✅ How it works:

Uses spliced and unspliced RNA reads to estimate directional cell transitions.
Models RNA metabolism dynamics to predict future cell fates.

✅ Best for:

Highly dynamic cellular processes (e.g., stress responses).
Predicting the next state of a cell rather than just ordering it in pseudotime.

❌ Weaknesses:

Requires spliced/unspliced counts (not available in standard Seurat RNA data).
Sensitive to preprocessing errors, incorrect velocity estimation leads to artifacts.
Often difficult to interpret.

📌 Use Case: Tracking stress adaptation in plant cells.
]
.pull-right[
"... vascular and mesophyll tissue share a common origin, the ground meristem. It has been previously challenging to profile such cells, but our trajectory inference delineated distinct cell fate decisions that distinguish the mesophyll and vasculature (Lopez-Anido et al., 2021)".
![scVelo Figure](Figures/scVelo_leaf.png)
]
---
...
.pull-right[
```{r slingshot authors 3, out.width= "120%", echo = FALSE}
knitr::include_graphics("Figures/slingshot_authors.png")
```
]
---
...
More than 13000 significant genes have been identified. 

---
#Finding genes that change as a function of pseudotime

.pull-left[
![DEG Figure](Figures/monocle3_sig_genes.png)
]

.pull-right[
![Pseudo Genes](Figures/monocle3_sig_genes_profiles.png)
]
---
#Identify Gene Modules Driving Transitions
```{r Modules, eval = FALSE}
gene_module_df<-find_gene_modules(cds[sig_genes,])
cell_group_df <- tibble::tibble(cell=row.names(colData(cds)), cell_group = clusters(cds))
agg_mat <- aggregate_gene_expression(cds, gene_module_df, cell_group_df)
pheatmap::pheatmap(agg_mat, scale="column", clustering_method="ward.D2"
```

```{r gene modules, out.width= "35%", echo = FALSE}
knitr::include_graphics("Figures/monocle3_modules.png")
```
---
class: middle, inverse
...
📌Clustering procedure identifies cell types and sub-cell types
📌Pseudotime orders cells in the developmental trajectories.

![Pseudo Genes](Figures/pseudospatial.png)
---
#Pseudospatial transcriptomics: root cell atlas

![Pseudospatial](Figures/RCA_1.png)
---
#Pseudospatial transcriptomics: root cell atlas

![Pseudospatial](Figures/RCA_2.png)
---
#Pseudospatial transcriptomics: root cell atlas

![Pseudospatial](Figures/RCA_3.png)
---
#RootCellAtlas web-service

![Pseudospatial](Figures/RootCellAtlas_starting.png)
...
```{r RCA, out.width= "55%", echo = FALSE}
knitr::include_graphics("Figures/RootCellAtlas.png")
```

rootcellatlas.org 
---
#Pseudospatial transcriptomics for other plant organs

.pull-left[
```{r emryo, out.width= "100%", echo = FALSE}
knitr::include_graphics("Figures/globular_embryo.png")
```
]

.pull-right[
```{r SAM, out.width= "80%", echo = FALSE}
knitr::include_graphics("Figures/SAM.png")
```
]
---
class: middle, inverse
...
---
#Integrate scTranscriptomics data with Other Data Modalities

Integration with other modalities helps to:

✅ Link transcriptional changes to chromatin accessibility (ATAC-Seq).

    Tools: Cicero (Monocle3) – infer enhancer activity;
    Signac (Seurat) – integrate scATAC-Seq with scRNA-Seq;
    ChromVAR
    
✅ Understand how cell-cell communication changes over pseudotime (Ligand-Receptor Analysis).

✅ Validate  the findings using spatial transcriptomics.

✅ Correlate metabolite and protein abundance with gene expression.

---
#Integrate scTranscriptomics data with Other Datasets
.pull-left[
Rare cell types can be found only via sample enrichment or data integration.
I.e. we were only able to detect stem cell clusters for the root when integrated 7 sc-datasets:
```{r Sstemcell, out.width= "100%", echo = FALSE}
knitr::include_graphics("Figures/stemcells.png")
```
]
.pull-right[
Inter-species comparison reveals conserved and divergent root developmental pathways (Zhang et al., Nat Comm, 2021):
```{r rice, out.width= "100%", echo = FALSE}
knitr::include_graphics("Figures/rice-ara.png")
```
]
---
class: middle, inverse
...
- There is no single ‘correct’ trajectory or cell cluster —biological insights come from comparing different approaches.
- A well-annotated dataset is more powerful than the most complex model.
- Reproducibility is key — always document your workflow and parameters
- Use ChatGPT (or other AI tools) to facilitate your coding experience
- **Do not hesitate to contact: victoria.mironova@ru.nl**
